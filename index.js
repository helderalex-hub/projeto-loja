...
async function processarEmailsVenda(venda) {
    ...
    const htmlCliente = `<div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e2e8f0;"><div style="background-color: #0f172a; padding: 40px 20px; text-align: center; border-bottom: 4px solid #cca43b;"><img src="${LOGO_URL}" alt="LUST STORE" style="max-width: 150px; height: auto; display: block; margin: 0 auto;"><p style="color: #cca43b; margin-top: 15px; font-size: 12px; letter-spacing: 4px; text-transform: uppercase;">Premium Beauty & Care</p></div><div style="padding: 40px 30px;"><h2 style="color: #0f172a; margin-top: 0; font-weight: 300;">Olá, ${venda.cliente_nome}.</h2><p style="color: #64748b; font-size: 16px; line-height: 1.5;">Agradecemos a sua preferência. A sua encomenda <b>#${venda.codigo_pedido}</b> foi confirmada.</p><div style="background: #f8fafc; padding: 15px; border-radius: 6px; margin: 20px 0; border: 1px solid #e2e8f0;"><p style="margin:0; font-size: 14px; color: #334155;">🚚 <strong>Estimativa de Entrega:</strong><br>${prazo} após envio.</p></div><h3 style="color: #0f172a; border-bottom: 1px solid #cca43b; padding-bottom: 10px; margin-top: 40px;">Resumo da Compra</h3><ul style="list-style: none; padding: 0; margin: 0;">${itensLista}</ul><div style="margin-top: 20px; text-align: right; border-top: 2px solid #f1f5f9; padding-top: 10px;"><p style="margin: 5px 0; color: #64748b;">Subtotal: €${(venda.total_venda - venda.total_frete).toFixed(2)}</p><p style="margin: 5px 0; color: #64748b;">Envio: €${venda.total_frete.toFixed(2)}</p><p style="font-size: 20px; color: #0f172a; margin-top: 10px;">Total: <b style="color: #cca43b;">€${venda.total_venda.toFixed(2)}</b></p></div></div><div style="background-color: #f1f5f9; padding: 20px; text-align: center; font-size: 12px; color: #94a3b8;"><p>© 2026 Lust Store. Todos os direitos reservados.</p></div></div>`;
    const htmlAdmin = `<h3>💰 NOVA VENDA: #${venda.codigo_pedido}</h3><p><b>Cliente:</b> ${venda.cliente_nome}</p><p><b>Total Pago:</b> €${venda.total_venda.toFixed(2)} (Frete: €${venda.total_frete})</p><p><b>Lucro Líquido:</b> <span style="color:green">€${venda.lucro.toFixed(2)}</span></p><hr><h4>Itens para Embalar:</h4><ul>${itensLista}</ul>`;
...
app.get('/config', async (req, res) => { const { data } = await supabase.from('config_loja').select('*').single(); res.json(data || { pt_std: 4.50, pt_exp: 8.00, pt_free: 60, es_std: 5.95, es_exp: 9.95, es_free: 85, eu_std: 12.50, eu_exp: 25.00, eu_free: 125 }); });
...
app.post('/checkout', async (req, res) => {
    try {
        const { itens, pais } = req.body; // RECEBE PAIS
