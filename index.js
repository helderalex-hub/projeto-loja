...
app.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        if (session.metadata && session.metadata.ids_produtos) {
            ...
            const valorTotalPago = session.amount_total;
            const valorFrete = session.total_details?.amount_shipping || 0;
            const valorVendaProdutos = valorTotalPago - valorFrete;
            ...
            await supabase.from('vendas').insert([{
                ...
                itens: itensVendidos, total_venda: valorTotalPago / 100, total_frete: valorFrete / 100,
                total_custo: custoProdutos, lucro: (valorVendaProdutos / 100) - custoProdutos
            }]);
        }
    }
