...
// --- WEBHOOK STRIPE (O Coração das Vendas) ---
app.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        if (session.metadata && session.metadata.ids_produtos) {
            ...
            let totalRecebido = session.amount_total / 100;
            let itensVendidos = [];
            ...
            // --- REGISTO DA VENDA COM DADOS DO CLIENTE ---
...
app.post('/checkout', async (req, res) => {
    try {
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            billing_address_collection: 'required', // Obriga o cliente a deixar a morada
